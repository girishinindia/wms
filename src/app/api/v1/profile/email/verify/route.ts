...
/**
 * POST /api/v1/profile/email/verify — confirm the code sent to the new
 * address. On success the email is updated, every session is revoked and
...
export async function POST(request: NextRequest) {
  return handler(async ({ requestId }) => {
    try {
      ...
      const result = await verifyContactChange(actor, "email", parsed.data.code, meta);
      if ("error" in result) {
        return fail("VALIDATION_FAILED", result.error, requestId, { fields: { code: result.error } });
      }
      return ok({ ok: true as const, signedOut: true as const, email: result.address }, requestId);
    } catch (error) {
